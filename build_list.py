#!/usr/bin/env python3
"""
Genera funcional.m3u: solo los canales dominicanos verificados, sin la lista
base de iptv-org. Lista corta y limpia para IPTV Smarters.

    python funcional.py                    genera funcional.m3u
    python funcional.py --check            verifica cada enlace
    python funcional.py --check --drop-broken   omite los que fallen
    python funcional.py --check --strict   falla el build si alguno cae

Codigos de salida:  0 ok | 1 error al escribir | 2 --strict con fallos

Telecentro pasa por el proxy (Cloudflare Worker) porque Telemicro exige
cabecera Referer y IPTV Smarters no manda cabeceras propias. Se configura con
la variable de entorno PROXY_BASE o con --proxy.
"""
...
def build_block(channel):
    ...
    return [
        "#EXTINF:-1 %s,%s" % (" ".join(attrs), channel["name"]),
        channel["url"],
    ]
...
def check_stream(channel):
    """
    master playlist -> variante -> segmento .ts. Comprobar solo el playlist no
    sirve: puede dar 200 mientras los segmentos fallan.
    """
    try:
...
def main():
    ap = argparse.ArgumentParser(description="Genera la lista corta de canales RD.")
    ...
    resumen = ["## Lista funcional", "", "- Canales: **%d**" % len(incluidos),
