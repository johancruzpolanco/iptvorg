...
# Proxy que anade el Referer por nosotros (Cloudflare Worker). Si se define,
# los canales con "proxy_path" pasan por el y dejan de necesitar cabeceras,
# que es lo unico que hace funcionar Telecentro en IPTV Smarters: Smarters
# ignora los #EXTVLCOPT y tampoco entiende el sufijo "|Referer=..." de la URL.
# Se configura con la variable de entorno PROXY_BASE o con --proxy.
#   ej: https://tc13.tu-usuario.workers.dev
...
def apply_proxy(proxy):
    """
    Reapunta al proxy los canales que lo tengan configurado. Al pasar por el
    proxy dejan de necesitar Referer/User-Agent, asi que se marcan sin
    cabeceras y la entrada de la lista queda limpia.
    """
    if not proxy:
        return 0
    n = 0
...
def main():
    ...
    ap.add_argument(
        "--proxy",
        default=PROXY_BASE,
        help="base del proxy que anade el Referer, ej: https://tc13.x.workers.dev "
        "(por defecto, la variable de entorno PROXY_BASE)",
    )
    args = ap.parse_args()
    ...
    if n:
        ...
        warn(
            "sin proxy configurado: Telecentro llevara cabeceras Referer, que "
            "IPTV Smarters ignora. Define PROXY_BASE o usa --proxy."
        )
